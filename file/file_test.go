...
func Test2(t *testing.T) {
	...
	fmt.Println(fmt.Sprintf("%x", timex.GetTimestamp(time.Now())))
	fmt.Println(fmt.Sprintf("%x", timex.GetTimestamp(time.Now().Add(time.Hour*24*365*185))))
...
func createCRCFileID(instanceId string, crc32 string, fileSize uint64) string {
	timestamp := fmt.Sprintf("%x", timex.GetTimestamp(time.Now()))
