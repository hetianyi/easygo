...
func (m *mock) Do() (interface{}, int, error) {
	...
	req, err := http.NewRequest(m.method, base.TValue(paramsStr == "", m.url, m.url+"?"+paramsStr).(string),
		base.TValue(isMultipart, pipeReader, bytes.NewReader(m.body)).(io.Reader))
	if err != nil {
